...
1. Explain what a token is used for.
   tokens are cryptic texts that are stored eather local or session storage. and it gets stored when ever user is logged in.
   Then the server can read the token and make decisions based on clients permission

2. What steps can you take in your web apps to keep your data secure?
   you can save your data by creating a token and saving it in the server everytime you log in.
   then only you will have permission to do certian actions based on your saved token.

3. Describe how web servers work.
   web servers work by getting the code that is sent in and then is able to hoist and share that code all around the world.
   but the web server needs code to tell it what to do.

4. Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
   axios.get: gets data
   axios.post: postes data
   axios.put: updates the current data
   axios.delete: deltes the data
